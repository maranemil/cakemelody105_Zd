...
// noinspection JSUnusedGlobalSymbols,JSUnusedLocalSymbols,JSDuplicatedDeclaration,JSUnresolvedVariable,JSUnresolvedFunction,JSIncompatibleTypesComparison,SpellCheckingInspection
...
String.prototype.parseColor = function () {
    ...
    if (this.slice(0, 4) === 'rgb(') {
        const cols = this.slice(4, this.length - 1).split(',');
        i = 0;
        do {
...
Effect.ScopedQueue = Class.create(Enumerable, {
    ...
    loop: function () {
        ...
        let i = 0, len = this.effects.length;
        for (; i < len; i++)
...
// Bug in opera makes the TD containing this element expands for an instance after finish
...
Effect.Pulsate = function (element) {
    ...
    let options = arguments[1] || {};
    let oldOpacity = element.getInlineOpacity();
    let transition = options.transition || Effect.Transitions.sinoidal;
    let reverser = function (pos) {
...
String.prototype.parseStyle = function () {
    ...
    if (Prototype.Browser.IE && this.include('opacity'))
        styleRules.set('opacity', this.match(/opacity:\s*([01]?(?:\.\d*)?)/)[1]);
