// noinspection JSPotentiallyInvalidUsageOfThis,JSPotentiallyInvalidConstructorUsage,JSUnresolvedFunction,SpellCheckingInspection
...
function storeimages(t) {
    let s = (t.getAttribute('src')) ? t.getAttribute('src') : '';
    let d = (t.getAttribute('srcdown')) ? t.getAttribute('srcdown') : '';
    let o = (t.getAttribute('srcover')) ? t.getAttribute('srcover') : '';
    ih.store(s, d, o);
}

function preloadimgsrc() {
    if (!document.getElementById) return;
    let it = document.getElementsByTagName('IMG');
    let it2 = document.getElementsByTagName('INPUT');
    preloader(it);
    preloader(it2);
}
